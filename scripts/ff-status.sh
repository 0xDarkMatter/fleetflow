...
# Lanes are read by FLEETFLOW_STATUS_WORKERS subshells at once (ADR-039); the
# output is journal first-appearance order whatever the worker count.
...
ENV
  ...
  FLEETFLOW_STATUS_WORKERS   lanes are read by this many workers at once (nproc, cap 8;
                             1 = serial). Output is identical at any count.
...
# Worker count for the per-lane loop (see lane_record / emit). Default: one
# per core, capped at 8 - past that the box is contending on the same disk
# and MSYS fork cost per worker, not on lane work (measured 2026-09-10 on a
# 32-core box: 4 -> 8 halved the 200-lane fixture again, 16 gained nothing).
# 1 is the serial path and is what the suite runs the byte-for-byte fixture
# comparison against.
STATUS_WORKERS="${FLEETFLOW_STATUS_WORKERS:-}"
[ -n "$STATUS_WORKERS" ] || STATUS_WORKERS="$(nproc 2>/dev/null | tr -d '\r' || echo 4)"
echo "$STATUS_WORKERS" | grep -qE '^[1-9][0-9]*$' || { err "FLEETFLOW_STATUS_WORKERS must be a positive integer"; exit 2; }
[ "$STATUS_WORKERS" -le 8 ] || STATUS_WORKERS=8
...
# === SECTION: lane_record - one lane's status record, inside a worker =========
# lane_record - compute ONE lane's record and write it to $LANEDIR/<idx>.json,
# plus an <idx>.stalled marker when the lane trips the --exit-stalled verdict.
#
# Runs INSIDE a worker subshell forked by emit(), never in the main shell: the
# run-level context (RUNDIR, RUN, REPO, now, WT_INTENT, integ_sha, base_sha,
# the primed MT stat cache) arrives by fork, and NOTHING this function sets is
# visible to emit afterwards. That is why its two outputs travel as files - a
# subshell's STALL_ANY=1 dies with the subshell. Do not "tidy" this back into
# emit's loop body or make it return values through variables: the parallel
# driver depends on the file contract and on this function touching no shared
...
lane_record() {
  ...
  # The stall verdict leaves this subshell as a FILE, not a variable: the
  # worker's STALL_ANY dies with the fork, and emit reduces the markers after
  # `wait` (a stalled lane must still make --exit-stalled exit 14).
  [ "$STALL_ANY" = 1 ] && : > "$LANEDIR/$idx.stalled"
  # `-n` (null input) is load-bearing, not habit: this runs inside a worker's
  # `while read` over its chunk file, and a jq that read stdin would swallow
  # the worker's remaining lanes.
  jq -nc \
    ...
    '{id:$id,model:$model,phase:$phase,state:$state,started:$started,elapsed_s:$elapsed,
      ...
      permission_denials:$denials,landed:$landed,commits_authored:$authored}' > "$LANEDIR/$idx.json" \
    || { err "lane $id: failed to serialise status record"; return 1; }
}

emit() {
  local now lanedir row nrows w p idx i f nrec failed
  ...
  # Set by the marker reduction below, never by the workers directly.
  STALL_ANY=0
  prime_mtimes
  ...
  now=$(date +%s)
  # Lane records accumulate in FILES, one JSON object per lane under a temp
  # DIR - NEVER in a shell variable. Folding each lane into a growing `$L + [...]`
  # array reads better and is exactly what this replaced: that form passed the
  # whole accumulated array back through jq's argv once per lane, so argv bytes
  # were QUADRATIC in lane count. Past ~37 lanes it exceeded the Windows
  # command-line limit, jq died with "Argument list too long", `$(...)` collapsed
  # the accumulator to the empty string, and every remaining lane then failed
  # `--argjson L ""`. emit produced nothing at all, so the machine-wide dashboard
  # rendered its three LARGEST runs (godaddy-build at 123 lanes, newbook-v1,
  # tess-v1) as empty "could not read this run" cards while every small run
  # looked fine (2026-09-09).
  # argv now carries exactly one lane's fields regardless of lane count, and the
  # array is assembled by slurping the concatenation on stdin - a redirection
  # bash performs, so no temp path is ever handed to jq (jq here is a native
  # Windows binary that would not resolve an MSYS /tmp path).
  # Do NOT "simplify" this back into a variable.
  #
  # One file PER LANE, not one shared file, because the lanes are read by
  # STATUS_WORKERS subshells at once (ADR-039): two writers appending to one
  # file interleave past the pipe buffer and tear records. Names are the
  # zero-padded journal index so the assembly below can walk them in journal
  # first-appearance order - output ordering is unchanged from the serial loop.
  # The dir is per emit and removed on every path (--watch calls emit per tick;
  # a leak here is a leak per tick), and the EXIT trap covers an interrupt.
  lanedir="$(mktemp -d)" || { err "cannot create temp dir for lane records"; return 1; }
  LANEDIR="$lanedir"; EMIT_TMPDIR="$lanedir"
  ...
  #
  # Rows are 0x1f-separated (see the worker `read` below for why not tab). Safe
  # because every field is an id, a model name, a phase name, a record type, an
  # integer rc, a pid, or a filesystem path - none of which can contain 0x1f.
  jq -sr '
      . as $all
...
# No field here can legitimately contain a CR.
  # Deal the rows ROUND-ROBIN into one chunk file per worker, prefixing each
  # with its journal index. Round-robin, not contiguous blocks: in-flight lanes
  # cost several spawns more than finished ones (transcript scan, spawner
  # probe) and a run's live lanes cluster at the journal's tail, so contiguous
  # chunks would hand one worker all the expensive rows. Pure bash - no process
  # per row - so this costs microseconds per lane, not a spawn.
  nrows=0
  while IFS= read -r row; do
    [ -n "${row%%$'\037'*}" ] || continue
    printf '%05d\037%s\n' "$nrows" "$row" >> "$lanedir/chunk-$((nrows % STATUS_WORKERS))"
    nrows=$((nrows + 1))
  done < "$lanedir/rows"
  # One SUBSHELL per worker, each walking its chunk serially. Subshells, not
  # `xargs bash -c`: a fork inherits the primed MT stat cache, every function
  # and the run-level context for free, where a fresh bash per lane would pay
  # a process spawn (~150ms here) just to start, then re-stat everything. The
  # worker count bounds the extra processes at STATUS_WORKERS per emit, not
  # per lane. Workers write only to $LANEDIR and stderr - never stdout.
  #
  ...
  failed=0
  for p in "${WORKER_PIDS[@]}"; do wait "$p" || failed=1; done
  WORKER_PIDS=()
  # --- reduce -----------------------------------------------------------------
  # The stall verdict is the OR of the per-lane markers; a worker's own
  # STALL_ANY never leaves its subshell. Completeness is checked by COUNT: a
  # worker that died mid-chunk leaves a gap, and a status that silently drops
  # lanes is the exact failure the argv-overflow incident taught (a partial
  # answer that exits 0 is worse than no answer).
  for f in "$lanedir"/*.stalled; do [ -e "$f" ] && { STALL_ANY=1; break; }; done
  nrec=0
  for f in "$lanedir"/*.json; do [ -e "$f" ] && nrec=$((nrec + 1)); done
  if [ "$failed" = 1 ] || [ "$nrec" != "$nrows" ]; then
    err "lane records incomplete ($nrec of $nrows) - a status worker failed"
    rm -rf "$lanedir"; EMIT_TMPDIR=""
    return 1
  fi
  # Concatenate by journal index with bash builtins only. Not `cat *.json`: a
  # glob walk is locale-collated and hands N paths to one argv - the very
  # thing this file's history says not to trust past a few hundred lanes.
  # Each record is one jq -c line, so the inner loop runs once per file.
  # Written AFTER the *.json completeness count above: its name would match
  # that glob, so creating it earlier would count it as a lane.
  : > "$lanedir/lanes.ndjson"
  for ((i = 0; i < nrows; i++)); do
    printf -v f '%s/%05d.json' "$lanedir" "$i"
    while IFS= read -r row; do printf '%s\n' "$row"; done < "$f"
  done >> "$lanedir/lanes.ndjson"
  local manifest="null"
  ...
  if ! jq -sc --arg run "$RUN" --arg repo "$REPO" --argjson now "$now" --arg orch "$orch" \
    ...
    '{run:$run,repo:$repo,generated_at:$now,stall_seconds:$stall,abandon_seconds:$abandon,
      ...
      lanes:.,manifest:$manifest}' < "$lanedir/lanes.ndjson"; then
...
STALL_ANY=0
# The per-emit temp dir is removed on every return path inside emit; the trap
# covers the one path emit cannot - ctrl-c mid-tick under --watch. Workers go
# BEFORE the dir: a background subshell inherits SIGINT *ignored* (bash's rule
# for async lists in a non-interactive shell), so a ctrl-c that ends this
# shell would otherwise leave them finishing their chunk into a dir that is
# already gone - harmless, but one missing-file error per lane on the way out.
EMIT_TMPDIR=""; WORKER_PIDS=()
emit_cleanup() {
  [ "${#WORKER_PIDS[@]}" -gt 0 ] && kill "${WORKER_PIDS[@]}" 2>/dev/null
  [ -n "${EMIT_TMPDIR:-}" ] && rm -rf "$EMIT_TMPDIR"
  return 0
}
trap emit_cleanup EXIT
