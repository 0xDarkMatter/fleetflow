...
| List section slugs / see where the time goes | `bash tests/run.sh --list` · `FLEETFLOW_TEST_TIMING=1 bash tests/run.sh` |
...
Only the no-flag run lands anything. Subset lanes exist to shorten the edit-run
loop (the dashboard lane is ~5s against ~11min) and tag their count
`(quick lane - not a landing gate)` so a pasted log can never be mistaken for
the gate. Sections share fixtures, so a subset also runs its prerequisites from
a table generated by `tests/section-deps.py` — see that file and the header of
`tests/run.sh`.
...
| `tests/` | `run.sh` — the one gate; run it before landing anything. `section-deps.py` derives the section prerequisite map `run.sh` embeds, so subset lanes (`--only` / `--quick`) can be trusted |
...
- **Only the no-flag `bash tests/run.sh` is the gate, and adding a section to it
  has two steps.** Every section is wrapped in `if __sec <slug>; then ... fi`,
  and the `__DEPS` prerequisite map it consults is GENERATED — a new section
  needs its wrapper plus `python tests/section-deps.py --emit` re-embedded, or
  `--check` fails and every subset lane refuses to run. Subset lanes
  (`--only` / `--skip` / `--quick`) are for iteration only: they print
  `(quick lane - not a landing gate)` and must never be pasted as evidence that
  something is ready to land. The store-isolation guard section runs in every
  mode by construction — never let a selection drop it.
