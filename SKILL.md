---
name: fleetflow
description: "Heterogeneous cross-provider fleet - GLM (z.ai), Codex (OpenAI), Grok (xAI), Pi (15+ providers), Anthropic Sonnet/Opus/Haiku - from one session, porting the native Workflow tool's patterns (adversarial verify, judge panels, journal resume) to OS-process workers. Triggers: fleetflow, heterogeneous/mixed-model fleet, codex worker, grok worker, cross-provider fan-out, cross-model verify."
...
`ff-clean.sh --run NAME [--force]` reclaims lanes with no unmerged commits
(worktree + branch deleted), keeps lanes still holding work the integration
branch lacks, and removes the run's cache dirs.
...
| Script | Purpose |
|---|---|
| [scripts/ff](scripts/ff) | the dispatcher: `ff <cmd>` forwards to `scripts/ff-<cmd>.sh` VERBATIM (sugar, never a layer - no forwarded command changes behaviour), so every `bash scripts/ff-*.sh` below shortens to `ff *` once it is on PATH; native conveniences with no script of their own: `ff env` (the `FLEETFLOW_*` registry, columnised), `ff open` (dashboard for this repo), `ff logs RUN ID` (tail a lane's artifacts), `ff watch RUN` (terminal live view, exits when every lane is final); tab-completion in [completions/ff.bash](completions/ff.bash) |
...
| [scripts/ff-doctor.sh](scripts/ff-doctor.sh) | `--offline` structural preflight (+ which `windows.sandbox` mode codex lanes will get); `--live` probes GLM endpoint, Codex auth, Grok key, Anthropic models, the `windows.sandbox` key tripwire, and reports orchestrator tier (fable/opus); `--env` prints the `FLEETFLOW_*` registry (name/current/default/purpose - the source of truth [docs/REFERENCE.md](docs/REFERENCE.md) mirrors, drift-gated both ways) |
...
| [scripts/ff-collect.sh](scripts/ff-collect.sh) | per-model result gate; strips ```json fences before `--schema` validation; `--repair` respawns a `<id>-repair` lane on validation failure; `--auto-commit` commits a dirty lane tree after a PASS so landing has a HEAD (opt-in, never changes the verdict); `--check-main-clean` escape guard; when a PASS resolves the last outstanding packet, an end-of-run summary (lane counts only - totals stay ff-status's) prints on stderr |
| [scripts/ff-status.sh](scripts/ff-status.sh) | run status as JSON (lane state `running`/`stalled`/`abandoned`/`done`/`failed`, elapsed, `last_activity_s` + `stalled` + `live_signal` stall detector, hours-scale abandonment demotion, commits, tools, tokens, activity, manifest summary); lanes are read in parallel by `FLEETFLOW_STATUS_WORKERS` subshells (nproc, cap 8; output is journal order at any count - ADR-039); `--watch N --out status.json` feeds the live monitor; `--exit-stalled` exits 14 so a watchdog can branch without parsing |
...
| [scripts/ff-clean.sh](scripts/ff-clean.sh) | `--run NAME [--force]` reclaims zero-commit lanes (worktree remove + branch -D), keeps lanes with unmerged commits, removes the run's cache dirs; reports locked ACL-litter dirs. Note a **landed** lane counts as zero-commit — commits are counted against the integration branch, never the manifest's frozen-sha base ([ADR-035](docs/adr/ADR-035-landedness-is-ancestry-in-the-integration-branch.md)) — so landing then cleaning reclaims it with no extra flag, merge and fast-forward alike ([ADR-020](docs/adr/ADR-020-sweep-reclaims-only-archived-and-landed.md)). **Archives the run to history first** (`--no-archive` opts out). `--reap [--force]` finds worker processes the wrapper left alive, matched by ancestry to the run's journalled anchors |
